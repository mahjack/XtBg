 ...
 * Time: 9:01
 */
public class Attendance_Leave_Detail extends Activity {
    ...
    LinearLayout llNeedHide;// 在休假当中是主管领导人，需要隐藏的部分
    ...
    private String xiujia_or_qingjia = "";
    ...
    private void initConstant() {
        mContext = Attendance_Leave_Detail.this;
        ...
        xiujia_or_qingjia = intent.getStringExtra("qingxiujia");
        KLog.d("请假还是休假", xiujia_or_qingjia);
...
//                    document_process_message.setVisibility(View.GONE);
        ...
        try {
            if (xiujia_or_qingjia.equals("xiujia")) {
                headerTitle.setText("休假查询");
            } else if (xiujia_or_qingjia.equals("qingjia")) {
                headerTitle.setText("请假查询");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        save_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (addJpgSignatureToGallery(signatureBitmap)) {
                    Toast.makeText(mContext, "签名已保存", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(mContext, "不能保存签名", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    void selectReceiver() {
        new AttendanceAuditDialog(mContext)
                .readData(ConstantURL.ATTENDANCE_LEAVE_AUDIT_USER + uid + "&id=" + detailId + "&do=show")
                .builder()
    ...
    void requestDetail() {
        try {
            mCall = OkHttpUtils
                    .get()
                    .url(ConstantURL.ATTENDANCE_LEAVE_DETAIL + uid + "&id=" + detailId)
                    .build();
...
//        KLog.d("tag", "审批页面，查询状态URL------------------>" + ConstantURL.ATTENDANCE_LEAVE_REQUEST_STATE + uid + "&id=" + detailId + "&do=show");
        mCall = OkHttpUtils
                .get()
                .url(ConstantURL.ATTENDANCE_LEAVE_REQUEST_STATE + uid + "&id=" + detailId + "&do=show")
                .build();
        mCall.execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e) {
    ...
    private void submit() {
        ...
        httpUtils.send(HttpRequest.HttpMethod.POST, ConstantURL.ATTENDANCE_LEAVE_AUDIT_ACTION + uid + "&id=" + detailId + "&do=do",
    ...
    private void submitSign() {
        ...
        params.addBodyParameter("tag", tag);
        params.addBodyParameter("totag", totag + "");
        params.addBodyParameter("userid", add_user_id);
        params.addBodyParameter("content", attendanceAuditEdt.getText().toString());
        params.addBodyParameter("status", attendance_status);
        if (isMessage!=0){
            params.addBodyParameter("duanxin", isMessage+"");
